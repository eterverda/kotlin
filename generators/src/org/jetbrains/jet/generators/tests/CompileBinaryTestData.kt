...
private annotation class GenScript(val dir: String, val source: String, vararg val binary: String)
...
fun main(args: Array<String>) {
    ...
    for (method in javaClass<BinaryTestData>().getDeclaredMethods()) {
        val script = method.getAnnotation(javaClass<GenScript>()) as? GenScript
        ...
        cwd = File(script.dir)
        for (file in script.binary) {
            rm(file)
        }

        val src = script.source
        // We assume that ${script.source}.txt file exists and contains the source to compile
        mv("$src.txt", src)
        try {
